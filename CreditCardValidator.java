...
//takes a look at all the even places of the credit card and doubles them, after it doubles those them it adds all of the single digits
		...
		String creditCard = String.valueOf(number);
			if (getSize(number) % 2 == 0) {
				...
				for(int i = getSize(number); i < 0 ; i -= 2 ) {
					...
					int total = getDigit((doubling - '0') * 2);
					sum += total;
					}
			}
